...
use crate::{asset_loader::SceneAssets, collision_detection::{self, CollisionDamage}, health::Health, movement::{self, MovingObjectBundle}, schedule::InGameSet, state::GameState};
...
fn spawn_spaceship(mut commands: Commands, scene_assets: Res<SceneAssets>) {
  commands.spawn((
    movement::MovingObjectBundle {
      velocity: movement::Velocity::new(Vec3::ZERO),
      accelaration: movement::Acceleration::new(Vec3::ZERO),
      collider: collision_detection::Collider::new(SPACESHIP_RADIUS),
      model: SceneBundle {
...
fn spaceship_movement_controls(
  mut query: Query<(&mut Transform, &mut movement::Velocity), With<Spaceship>>,
...
fn spaceship_weapon_controls(
  ...
  if keyboard_input.pressed(KeyCode::Space) {
    commands.spawn(
      (
        MovingObjectBundle {
          velocity: movement::Velocity::new(-transform.forward() * MISSILE_SPEED),
          accelaration: movement::Acceleration::new(Vec3::ZERO),
          collider: collision_detection::Collider::new(MISSILE_RADIUS),
          model: SceneBundle {
